...
fn main() {
    ...
    // --- 1b. H_S with stack-local AES (no TLS) ---
    let aes_local = Aes128::new(GenericArray::from_slice(b"HT-IDPF-HALFTREE"));
    for s in seeds.iter().take(WARMUP) {
        std::hint::black_box(prg::half_tree_hs(&aes_local, &s.key));
    }
    let start = Instant::now();
    for s in seeds.iter() {
        std::hint::black_box(prg::half_tree_hs(&aes_local, &s.key));
    }
    let ns_hs_stack = start.elapsed().as_nanos();
    print_row("H_S via stack-local AES (no TLS)", ns_hs_stack);
    ...
    let ns_tls_only = ns_hs_tls.saturating_sub(ns_hs_stack);
    let ns_pipeline = ns_full.saturating_sub(sum_parts + ns_tls_only);
    println!("Sum of isolated parts (sigma+AES+xor+struct+copy+right): {:.3} ms ({:.1} ns/call)",
        sum_parts as f64 / 1_000_000.0, sum_parts as f64 / ITERATIONS as f64);
    println!("TLS lookup overhead (H_S TLS - H_S stack):               {:.3} ms ({:.1} ns/call)",
        ns_tls_only as f64 / 1_000_000.0, ns_tls_only as f64 / ITERATIONS as f64);
    println!("Pipeline / dependency overhead (remainder):               {:.3} ms ({:.1} ns/call)",
        ns_pipeline as f64 / 1_000_000.0, ns_pipeline as f64 / ITERATIONS as f64);
    println!("Full expand() measured:                                   {:.3} ms ({:.1} ns/call)",
        ns_full as f64 / 1_000_000.0, ns_full as f64 / ITERATIONS as f64);
    println!("Projected expand() without TLS (total - TLS):            {:.3} ms ({:.1} ns/call)",
        (ns_full - ns_tls_only) as f64 / 1_000_000.0, (ns_full - ns_tls_only) as f64 / ITERATIONS as f64);
}
